...
export default function StatsPage() {
  ...
  if (!stats) {
    return (
      <div className="min-h-screen bg-[#050505] flex items-center justify-center">
        <p className="text-[#888]">Failed to load statistics</p>
      </div>
    )
  }
      ...
      <div className="max-w-6xl mx-auto px-6 py-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
          <div className="bg-white/[0.02] rounded-2xl p-6 border border-white/[0.04]">
            <p className="text-[#888] text-sm uppercase tracking-wider mb-2">Total Players</p>
            <p className="text-4xl font-bold text-white">{stats.totalPlayers}</p>
          </div>
          <div className="bg-white/[0.02] rounded-2xl p-6 border border-white/[0.04]">
            <p className="text-[#888] text-sm uppercase tracking-wider mb-2">Total Ratings</p>
            <p className="text-4xl font-bold text-white">{stats.totalRatings}</p>
          </div>
          <div className="bg-white/[0.02] rounded-2xl p-6 border border-white/[0.04]">
            <p className="text-[#888] text-sm uppercase tracking-wider mb-2">Active Voters</p>
            <p className="text-4xl font-bold text-white">{stats.totalVoters}</p>
          </div>
        </div>

        {/* Class Statistics */}
        <div className="bg-white/[0.02] rounded-2xl border border-white/[0.04] p-6 mb-8">
          <h2 className="text-xl font-semibold text-white mb-6">Class Breakdown</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {Object.entries(stats.byCategory).map(([category, data]) => (
              <div key={category} className="bg-white/[0.02] rounded-xl p-5">
                <div className="flex items-center justify-between mb-4">
                  <span className="text-white font-medium">{category}</span>
                  <span className="text-white font-bold">{data.count} players</span>
                </div>
                <div className="flex items-end justify-between">
                  <span className="text-[#888] text-sm">Avg Rating</span>
                  <span className="text-2xl font-bold text-white">{data.avgRating.toFixed(1)}</span>
                </div>
                {/* Progress bar */}
                <div className="mt-3 h-2 bg-white/[0.03] rounded-full overflow-hidden">
                  <div 
                    className={cn(
                      "h-full rounded-full",
            ...
            <div className="flex-1 grid grid-cols-2 gap-3">
              {stats.ratingDistribution.map((bucket, idx) => (
                <div key={bucket.range} className="flex items-center gap-3">
                  ...
                  />
                  <span className="text-[#888] text-sm">{bucket.range}</span>
          ...
          <div className="bg-white/[0.02] rounded-2xl border border-white/[0.04] p-6">
            ...
            <div className="space-y-3">
              {stats.topClans.slice(0, 10).map((clan, idx) => (
                <div key={clan.name} className="flex items-center gap-4 bg-white/[0.02] rounded-lg p-3">
                  <span className={cn(
                    "w-6 text-center font-bold",
                    idx === 0 ? "text-white" :
                    idx === 1 ? "text-slate-300" :
                    idx === 2 ? "text-slate-400" : "text-[#555]"
                  )}>
                    {idx + 1}
                  </span>
                  {/* Clan Logo */}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <span className="text-[#555] text-xs">{clan.count} players</span>
                  </div>
                  <span className="text-white font-bold">{clan.avgRating.toFixed(1)}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="bg-white/[0.02] rounded-2xl border border-white/[0.04] p-6">
            ...
            <div className="space-y-3">
              {stats.topNationalities.slice(0, 10).map((nation, idx) => (
                <div key={nation.code} className="flex items-center gap-4 bg-white/[0.02] rounded-lg p-3">
                  <span className={cn(
                    "w-6 text-center font-bold",
                    idx === 0 ? "text-white" :
                    idx === 1 ? "text-slate-300" :
                  ...
                  <div className="flex-1">
                    <span className="text-[#555] text-sm">({nation.count} players)</span>
                  </div>
                  <span className="text-white font-bold">{nation.avgRating.toFixed(1)}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        {Object.keys(stats.byDivision).length > 0 && (
          <div className="bg-white/[0.02] rounded-2xl border border-white/[0.04] p-6 mt-8">
            ...
            <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
              {Object.entries(stats.byDivision).sort((a, b) => a[0].localeCompare(b[0])).map(([div, data]) => (
                <div key={div} className="bg-white/[0.02] rounded-xl p-4 text-center">
                  <div className="text-2xl font-bold text-white mb-1">Div {div}</div>
                  <div className="text-white text-lg font-semibold">{data.count}</div>
                  <div className="text-[#888] text-sm">players</div>
                  <div className="text-[#888] text-sm mt-2">Avg: {data.avgRating.toFixed(1)}</div>
                </div>
              ))}
            </div>
          </div>
